...
void doit(int fd) 
{
    ...
    // notice: save the uri here, if not, uri will be cached after parse_uri, then make mistake when cache
    char url[MAXLINE];
    strcpy(url, uri);
    
    // check cache
    // if exist, return without updating cache according to lru
    int i = cache_find(&cache, uri);
    if (i != -1) {
        read_requesthdrs(&rio_client);
        read_pre(&cache, i);
        strcpy(buf, cache.cacheobjs[i].obj);
        read_after(&cache, i);
    ...
    char cache_obj_buf[MAX_OBJECT_SIZE];
    int n, cnt = 0;
    while ((n = Rio_readlineb(&rio_server, buf, MAXLINE)) != 0) {
        printf("proxy received %d bytes\n", n);
        cnt += n;
        if (cnt <= MAX_OBJECT_SIZE)
            strcat(cache_obj_buf, buf);
        Rio_writen(fd, buf, n);
    } 
    Close(end_serverfd);
    if (cnt <= MAX_OBJECT_SIZE)
        cache_store(&cache, url, cache_obj_buf);
}

/*
 * read_requesthdrs - read HTTP request headers and ignore them
 */
void read_requesthdrs(rio_t *rp) 
{
    char buf[MAXLINE];
